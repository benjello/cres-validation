...
from cres_validation import colums_validator as colums_validator_module


def validate_csv_columns(
    csv_path: Path, delimiter: str = ",", table_name: str = "individu", column_mapping: dict = None
):
    """
    ...
    if table_name not in colums_validator_module.schema_by_table:
        available = list(colums_validator_module.schema_by_table.keys())
        raise ValueError(f"Schéma '{table_name}' introuvable. Schémas disponibles: {available}")

    schema = colums_validator_module.schema_by_table[table_name]
    ...
    try:
        ...
        if column_mapping:
            ...
            if "role_menage" in df_mapped.columns:
                ...
                df_mapped["role_menage"] = pd.to_numeric(
                    df_mapped["role_menage"], errors="coerce"
                ).astype("Int64")
                # Remplacer NaN par une valeur par défaut si nécessaire (0 pour role_menage)
                if df_mapped["role_menage"].isna().any():
                    print(
                        f"  ⚠️  {df_mapped['role_menage'].isna().sum()} valeur(s) null trouvée(s) dans role_menage"
                    )
...
if __name__ == "__main__":
    ...
    success = validate_csv_columns(
        csv_path, delimiter=",", table_name=table_name, column_mapping=column_mapping
    )
