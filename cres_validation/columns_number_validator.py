...
def analyze_csv_columns(
        ...
        with open(csv_path, encoding=enc, newline='', buffering=8192*16) as f:
            for _line_num, line in enumerate(f, start=1):
                ...
                if is_first_line:
                    ...
                    is_header = (
                        (stripped.startswith(delimiter) or stripped.startswith(';')) and
    ...
    if header_columns is not None and header_columns == expected_columns + 1:
        logger.debug(f"Header avec {header_columns} colonnes détecté (données: {expected_columns}). C'est normal.")
...
def correct_csv(
    ...
    def adjust_header(header_line: str, first_data_line: str, delimiter: str, expected_cols: int) -> str:
        """
        ...
        if header_cols == data_cols + 1:
            ...
            if header_parts[-1].strip() == '' or (data_cols > 0 and header_parts[-1].strip() == data_parts[-1].strip()):
                # Supprimer la dernière colonne du header
                adjusted_header = delimiter.join(header_parts[:-1])
                logger.info(f"Header ajusté: {header_cols} → {len(header_parts[:-1])} colonnes (suppression de la dernière colonne vide)")
        ...
        with open(csv_path, encoding=enc, newline='', buffering=8192*16) as infile, \
            ...
            for _line_num, line in enumerate(infile, start=1):
                ...
                if is_first_line:
                    ...
                    is_header = (
                        (stripped_first.startswith(delimiter) or stripped_first.startswith(';')) and
                        ('matricul' in stripped_first.lower() or 'cin' in stripped_first.lower())
                    )
                    if is_header:
                        header_line = line
                        is_first_line = False
                ...
                if header_line is not None and first_data_line is None:
                    ...
                    if stripped_data.strip():
                        ...
                        adjusted_header = adjust_header(header_line, first_data_line, delimiter, expected_columns)
